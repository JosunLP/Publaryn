...
- Manifest referrers API (`/v2/{name}/referrers/{digest}`) — returns
  `404` for now; cosign tolerates this.
...
- **Negative:** OCI spec has many optional features (referrers,
  chunk size negotiation, Warnings header, blob mount). MVP excludes the
