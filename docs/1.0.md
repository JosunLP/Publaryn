...
`GET /v1/stats` is part of the public 1.0 contract. It returns stable top-level
aggregate fields for quick instance-level visibility:
...
`GET /v1/admin/jobs` is the intentionally narrow platform-admin operator surface
for queue visibility and recovery triage. It requires a platform-administrator
identity with the `audit:read` scope, supports `state`, `kind`, `page`, and
`per_page` filters, and is operationally documented in
...
- authenticated callers can also see visible private and organization-internal
  packages they can access through owner rights, organization membership, or
