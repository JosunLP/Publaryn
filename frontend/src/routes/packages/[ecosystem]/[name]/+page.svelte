<script lang="ts">
  ...
  import { ecosystemIcon, ecosystemLabel, installCommand } from '../../../../utils/ecosystem';
  import { copyToClipboard, formatDate, formatNumber } from '../../../../utils/format';
  ...
  async function loadPackagePage(): Promise<void> {
    ...
    const [loadedReleases, loadedTags, loadedFindings, loadedTransferState] = await Promise.all([
      listReleases(eecosystem(), ename(), { perPage: 20 }).catch(() => [] as Release[]),
      listTags(eecosystem(), ename()).catch(() => [] as Tag[]),
      listSecurityFindings(eecosystem(), ename()).catch(() => [] as SecurityFinding[]),
  ...
  async function loadTransferState(currentPackage: PackageDetail | null): Promise<TransferState> {
    if (!currentPackage || !getAuthToken() || currentPackage.can_transfer !== true) {
      return {
        showTransfer: false,
        organizations: [],
        loadError: null,
    ...
    try {
      ...
      return {
        showTransfer: true,
        organizations: selectPackageTransferTargets(response.organizations || [], currentPackage.owner_org_slug),
        loadError: null,
      };
    } catch (caughtError: unknown) {
      return {
  ...
  async function handleCopyInstall(): Promise<void> {
    ...
    const latestVersion =
      pkg.latest_version ?? (releases.length > 0 ? releases[0]?.version ?? null : null);
  ...
  function latestVersionForPackage(currentPackage: PackageDetail): string | null {
    return currentPackage.latest_version ?? (releases.length > 0 ? releases[0]?.version ?? null : null);
  }
...
{:else if notFound}
  <div class="empty-state mt-6">
    ...
    <a href="/search" class="btn btn-primary mt-4" data-sveltekit-preload-data="hover">Search packages</a>
  </div>
{:else if error || !pkg}
  <div class="alert alert-error mt-6">Failed to load package: {error || 'Unknown error.'}</div>
{:else}
  {@const latestVersion = latestVersionForPackage(pkg)}
  {@const install = latestVersion ? installCommand(eecosystem(), pkg.name, latestVersion) : installCommand(eecosystem(), pkg.name)}

  <div class="mt-6">
    <div class="pkg-header">
      <h1 class="pkg-header__name">{pkg.display_name || pkg.name}</h1>
      <span class="badge badge-ecosystem">{ecosystemIcon(eecosystem())} {ecosystemLabel(eecosystem())}</span>
      {#if latestVersion}
    ...
    {#if pkg.description}
      <p class="text-muted mt-4" style="font-size:1.05rem;">{pkg.description}</p>
    {/if}
      ...
      <div class="pkg-detail__main">
        <div class="card mb-4">
          ...
          <div class="code-block">
            <code>{install}</code>
            <button class="copy-btn" type="button" on:click={handleCopyInstall}>Copy</button>
          </div>
        </div>

        <div class="tabs">
          <button class:active={activeTab === 'readme'} class="tab" type="button" on:click={() => (activeTab = 'readme')}>Readme</button>
          <button class:active={activeTab === 'versions'} class="tab" type="button" on:click={() => (activeTab = 'versions')}>Versions ({releases.length})</button>
          <button class:active={activeTab === 'security'} class="tab" type="button" on:click={() => (activeTab = 'security')}>
            Security
            {#if openFindings.length > 0}
              <span class={`badge badge-severity-${worstSeverity(openFindings)}`} style="margin-left:4px;">{openFindings.length}</span>
            {/if}
          </button>
        </div>
        ...
        {#if activeTab === 'security'}
          <div class="findings-toggle">
            <label>
              <input type="checkbox" bind:checked={includeResolvedFindings} on:change={handleResolvedToggleChange} />
          ...
          {:else}
            {#each [...findings].sort((left, right) => severityLevel(right.severity) - severityLevel(left.severity)) as finding}
              ...
              <div class={`finding-row ${finding.is_resolved ? 'finding-resolved' : ''}`}>
                <div class="finding-row__header">
                  <span class={`badge badge-severity-${severity}`}>{severity}</span>
                  <span class="badge badge-ecosystem">{formatFindingKind(finding.kind)}</span>
                  <span class="finding-row__title">{finding.title}</span>
                  {#if finding.advisory_id}
                    {#if finding.advisory_id.startsWith('CVE-')}
                      <a href={`https://nvd.nist.gov/vuln/detail/${encodeURIComponent(finding.advisory_id)}`} target="_blank" rel="noopener noreferrer">{finding.advisory_id}</a>
                    {:else}
                ...
                <div class="finding-row__meta">
                  {#if finding.release_version}<span>v{finding.release_version}</span>{/if}
                  {#if finding.artifact_filename}<span>{finding.artifact_filename}</span>{/if}
                  <span>{formatDate(finding.detected_at)}</span>
                  {#if finding.is_resolved}
                    <span>Resolved{finding.resolved_at ? ` ${formatDate(finding.resolved_at)}` : ''}</span>
                  {/if}
                </div>
                {#if finding.description}
                  <div class="finding-row__description">{finding.description}</div>
                {/if}
              </div>
            {/each}
          {/if}
      ...
      <div class="pkg-detail__sidebar">
        <div class="card">
          <div class="sidebar-section">
            <h3>Package info</h3>
            {#if pkg.license}<div class="sidebar-row"><span class="sidebar-row__label">License</span><span class="sidebar-row__value">{pkg.license}</span></div>{/if}
            {#if pkg.visibility}<div class="sidebar-row"><span class="sidebar-row__label">Visibility</span><span class="sidebar-row__value">{pkg.visibility}</span></div>{/if}
            {#if pkg.download_count != null}<div class="sidebar-row"><span class="sidebar-row__label">Downloads</span><span class="sidebar-row__value">{formatNumber(pkg.download_count)}</span></div>{/if}
            {#if pkg.created_at}<div class="sidebar-row"><span class="sidebar-row__label">Created</span><span class="sidebar-row__value">{formatDate(pkg.created_at)}</span></div>{/if}
            {#if pkg.updated_at}<div class="sidebar-row"><span class="sidebar-row__label">Updated</span><span class="sidebar-row__value">{formatDate(pkg.updated_at)}</span></div>{/if}
          </div>
        </div>
          ...
          <div class="card">
            <div class="sidebar-section">
              ...
              {#if pkg.owner_org_slug}
                <a href={`/orgs/${encodeURIComponent(pkg.owner_org_slug)}`} data-sveltekit-preload-data="hover">{pkg.owner_org_slug}</a>
              {:else if pkg.owner_username}
                <a href={`/search?q=${encodeURIComponent(pkg.owner_username)}`} data-sveltekit-preload-data="hover">{pkg.owner_username}</a>
              {/if}
            </div>
          </div>
        {/if}
            ...
            <div class="sidebar-section">
              ...
              <div>
                {#each ['critical', 'high', 'medium', 'low', 'info'] as severity}
                  {@const count = openFindings.filter((finding) => finding.severity?.toLowerCase() === severity).length}
                  {#if count > 0}
                    <span class={`badge badge-severity-${severity}`} style="margin-right:4px;">{count} {severity}</span>
                  {/if}
                {/each}
              </div>
            </div>
        ...
        {#if pkg.can_manage_releases}
          <div class="card">
            <div class="sidebar-section">
              ...
              {#if releaseNotice}<div class="alert alert-success" style="margin-bottom:12px;">{releaseNotice}</div>{/if}
              {#if releaseError}<div class="alert alert-error" style="margin-bottom:12px;">{releaseError}</div>{/if}
              <form id="package-create-release-form" on:submit={handleCreateRelease}>
                <div class="form-group" style="margin-bottom:12px;">
                  <label for="release-version">Version</label>
                  <input bind:value={newReleaseVersion} id="release-version" name="version" class="form-input" placeholder="1.0.0" required />
                </div>
                <div class="form-group" style="margin-bottom:12px;">
                  <label for="release-description">Description</label>
                  <textarea bind:value={newReleaseDescription} id="release-description" name="description" class="form-input" rows="2" placeholder="Optional release summary"></textarea>
                </div>
                <div class="form-group" style="margin-bottom:12px;">
                  <label for="release-changelog">Changelog</label>
                  <textarea bind:value={newReleaseChangelog} id="release-changelog" name="changelog" class="form-input" rows="4" placeholder="Optional changelog notes"></textarea>
                </div>
                <div class="form-group" style="margin-bottom:12px;">
                  <label for="release-source-ref">Source ref</label>
                  <input bind:value={newReleaseSourceRef} id="release-source-ref" name="source_ref" class="form-input" placeholder="refs/tags/v1.0.0" />
                </div>
                <div class="form-group" style="margin-bottom:12px;">
                  <label class="flex items-start gap-2">
                    <input bind:checked={newReleaseIsPrerelease} type="checkbox" name="is_prerelease" />
                    <span>Mark this version as a pre-release instead of relying on version suffix detection.</span>
                  </label>
                </div>
                <button type="submit" class="btn btn-primary" style="width:100%; justify-content:center;" disabled={creatingRelease}>
        ...
        {#if transferState.showTransfer}
          <div class="card">
            <div class="sidebar-section">
              ...
              {#if transferNotice}<div class="alert alert-success" style="margin-bottom:12px;">{transferNotice}</div>{/if}
              {#if transferError}<div class="alert alert-error" style="margin-bottom:12px;">{transferError}</div>{/if}
              {#if transferState.loadError}<div class="alert alert-error" style="margin-bottom:12px;">{transferState.loadError}</div>{/if}
              <p class="settings-copy" style="margin-bottom:12px;">
                Move this package away from {pkg.owner_org_slug || pkg.owner_username || 'the current owner'} into an organization you already administer.
              </p>
              ...
              {:else}
                <form id="package-transfer-form" on:submit={handleTransferPackage}>
                  <div class="form-group" style="margin-bottom:12px;">
                    <label for="package-transfer-target">Target organization</label>
                    <select bind:value={targetOrgSlug} id="package-transfer-target" name="target_org_slug" class="form-input" required>
                      <option value="">Select an organization</option>
                      {#each transferState.organizations as organization}
                        <option value={organization.slug || ''}>{organization.name || organization.slug || 'Unnamed organization'}</option>
                      {/each}
                    </select>
                  </div>
                  <div class="form-group" style="margin-bottom:12px;">
                    <label class="flex items-start gap-2">
                      <input bind:checked={transferConfirmed} type="checkbox" id="package-transfer-confirm" name="confirm" required />
                      <span>I understand this package transfer is immediate and existing team grants will be removed.</span>
                    </label>
                  </div>
                  <button type="submit" class="btn btn-danger" style="width:100%; justify-content:center;" disabled={transferringPackage}>
        ...
        {#if pkg.homepage || pkg.repository_url}
          <div class="card">
            <div class="sidebar-section">
              <h3>Links</h3>
              {#if pkg.homepage}<div style="margin-bottom:4px;"><a href={pkg.homepage} target="_blank" rel="noopener noreferrer">Homepage</a></div>{/if}
              {#if pkg.repository_url}<div style="margin-bottom:4px;"><a href={pkg.repository_url} target="_blank" rel="noopener noreferrer">Repository</a></div>{/if}
            </div>
          </div>
        {/if}
            ...
            <div class="sidebar-section">
              ...
              <div>
                {#each pkg.keywords as keyword}
                  <a href={`/search?q=${encodeURIComponent(keyword)}`} class="badge badge-ecosystem" style="margin:2px;" data-sveltekit-preload-data="hover">{keyword}</a>
                {/each}
              </div>
            </div>
          </div>
            ...
            <div class="sidebar-section">
              ...
              {#each tags as tag}
                <div class="sidebar-row">
                  <span class="sidebar-row__label">{tag.tag || tag.name || ''}</span>
